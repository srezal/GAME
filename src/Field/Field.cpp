...
Field::Field(unsigned int width, unsigned int height, const Vector& start_position, const Vector& finish_position, bool is_night){
    ...
    if(is_night) for(int i = 0; i < this->rows * this->cols; i++) cells[i].setVisible(0);
}
...
Field::Field(Field&& other_field): rows(0), cols(0), start_position(Vector(0, 0)), finish_position(Vector(0, 0)), cells(nullptr){
    ...
    std::swap(is_night, other_field.is_night);
...
Field& Field::operator = (const Field& other_field){
    ...
    std::swap(is_night, temp.is_night);
...
void Field::setIs_night(bool value){
    is_night = value;
}
